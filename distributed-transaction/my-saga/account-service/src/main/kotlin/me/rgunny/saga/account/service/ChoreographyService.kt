...
/**
 * 순수 Choreography Saga 패턴으로 계좌 이체를 처리하는 서비스.
 *
...
class ChoreographyService(
    ...
    fun initiateTransfer(request: TransferRequest): TransferResponse {
        ...
        // Step 1: 출금
        accountService.withdraw(sagaId, request.fromAccountNumber, request.amount)
    ...
    fun handleDepositFailed(event: DepositFailedEvent) {
        accountService.compensateWithdraw(event.fromAccountNumber, event.amount)
